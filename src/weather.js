var Main = {
  controller: function() {
    ...
    self.onHamburgerClick = function(argument) {
      oem.read('menuDrawer').toggle();
    };
    self.onDrawerClose = function(argument) {
      oem.read('hamburger').deactivate();
    };
    self.onWeatherFormSubmit = function(e) {
      self.getWeather(e.data.zipCode);
    };
    ...
    oem.events.addEventListener(oem.EVENTS.COMPONENTS_INITIALIZED, function() {
      oem
        .read('hamburger')
        .getEl()
        .addEventListener('click', self.onHamburgerClick);
      oem.events.addEventListener(
        oem.read('menuDrawer').getEvents().closed,
        self.onDrawerClose
      );
      oem.events.addEventListener(
        oem.read('weatherForm').getEvents().submitted,
        self.onWeatherFormSubmit
      );
    ...
    function getDayReport(day) {
      return m(
        ...
        day.map(function(report) {
          ...
          var temp = m(
            'span',
            { class: 'weather-list__temp' },
            m.trust(Math.round(report.main.temp) + '&deg; ')
          );
          var time = m(
            'span',
            { class: 'weather-list__time' },
            '@' + formatTime(report.dt_txt)
          );
          var icon = m('i', {
            class: 'wi ' + ctrl.iconList[report.weather[0].icon]
          });
            ...
            m(
              ...
              [
                ...
                m(
                  'div',
                  { class: 'help' },
                  'Enter your zip code to get the latest weather.'
                ),
                m('input', {
                  type: 'text',
                  name: 'zipCode',
